...
namespace ResetTerrainFeatures_NET6
{
    public static class Regenerator
    {
        ...
        {
            Clear(location, types, indices);
            LoadMapFeatures(location, types, indices);
        }

        public static int[] GetIndicesFromOptions()
        {
            List<int> list = new();
            List<string> list2 = regeneratorOptions.Keys.ToList();
            foreach (string item in list2)
            {
                bool flag = !regeneratorOptions[item] || !indexKeys.ContainsKey(item);
                if (!flag)
                {
                    foreach (int item2 in indexKeys[item])
                    {
                        list.Add(item2);
                    }
                }
        ...
        public static Type[] GetTypesFromOptions()
        {
            ...
            {
                string key = list2[i];
                bool flag = regeneratorOptions[key] && typeKeys.ContainsKey(key);
                if (flag)
                {
                    list.Add(typeKeys[key]);
                }
            }
            return list.ToArray();
        }
        ...
        public static void Clear(GameLocation location, Type[] types = null, int[] parentSheetIndices = null)
        {
            ...
            foreach (Vector2 key in location.terrainFeatures.Keys)
            {
                ...
                bool flag = regeneratorOptions.ContainsKey("Crop") && regeneratorOptions["Crop"] && terrainFeature is HoeDirt;
                if (flag)
                {
            ...
            foreach (Vector2 key2 in location.objects.Keys)
            {
                ...
                bool flag3 = types == null || types.Contains(@object.GetType()) || parentSheetIndices != null && parentSheetIndices.Contains(@object.ParentSheetIndex) || types.Contains(typeof(StardewValley.Object)) || regeneratorOptions.ContainsKey("Forage") && regeneratorOptions["Forage"] && @object.isForage();
                if (flag3)
                {
                    list2.Add(key2);
                }
        ...
        {
            GameLocation gameLocation = MakeClone(location);
            List<ResourceClump> clumps = GetClumps(location);
            ...
            {
                bool flag2 = (types == null || types.Contains(pair2.Value.GetType()) || parentSheetIndices != null && parentSheetIndices.Contains(pair2.Value.ParentSheetIndex) || types.Contains(typeof(StardewValley.Object)) || regeneratorOptions.ContainsKey("Forage") && regeneratorOptions["Forage"] && pair2.Value.isForage()) && !location.terrainFeatures.ContainsKey(pair2.Key) && !location.objects.ContainsKey(pair2.Key);
                if (flag2)
                {
